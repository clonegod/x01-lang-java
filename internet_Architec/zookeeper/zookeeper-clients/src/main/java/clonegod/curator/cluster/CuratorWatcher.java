...
/**
 * 使用 PathChildrenCache 监控 /configs 的子节点变化
 *  ------ 共享配置 ，配置中心， 实时通知客户端服务地址的变更
 */
public class CuratorWatcher {
	...
	public CuratorWatcher() throws Exception{
		...
		cache = new PathChildrenCache(cf, PARENT_PATH, true);
		//5 在初始化的时候就进行缓存监听
		...
		cache.getListenable().addListener(new PathChildrenCacheListener() {
			...
			public void childEvent(CuratorFramework cf, PathChildrenCacheEvent event) throws Exception {
				...
				case CHILD_ADDED:
					System.out.println("CHILD_ADDED, path:" + event.getData().getPath() + 
							", data:" + new String(event.getData().getData()));
					break;
				case CHILD_UPDATED:
					System.out.println("CHILD_UPDATED, path:" + event.getData().getPath()
							+", data:" + new String(event.getData().getData()));
					break;
				case CHILD_REMOVED:
					System.out.println("CHILD_REMOVED, path:" + event.getData().getPath()
							+", data:" + new String(event.getData().getData()));
					break;
				default:
					break;
